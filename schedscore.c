...
static void dump_migrations_summary_table(struct schedscore_bpf *skel)
{
	...
	int w_id=0, w_tot=0, w_rw=0, w_lb=0, w_n=0, w_lc=0, w_ll=0, w_lx=0; char buf[64];
	key = next = 0;
	while ((err = bpf_map_get_next_key(stats_fd, &key, &next)) == 0) {
		struct schedscore_paramset_stats st;
		if (bpf_map_lookup_elem(stats_fd, &next, &st) == 0) {
			...
			snprintf(buf,sizeof buf, "%llu", l_c); l=strlen(buf); if (l>w_lc) w_lc=l;
			snprintf(buf,sizeof buf, "%llu", l_l); l=strlen(buf); if (l>w_ll) w_ll=l;
			snprintf(buf,sizeof buf, "%llu", l_x); l=strlen(buf); if (l>w_lx) w_lx=l;
		}
		key = next;
	}
	/* Minimums to fit labels */
	if (w_id < (int)strlen("paramset_id")) w_id = (int)strlen("paramset_id");
	if (w_tot < (int)strlen("migr_total")) w_tot = (int)strlen("migr_total");
	if (w_rw < (int)strlen("migr_wakeup")) w_rw = (int)strlen("migr_wakeup");
	if (w_lb < (int)strlen("migr_lb")) w_lb = (int)strlen("migr_lb");
	if (w_n < (int)strlen("migr_numa")) w_n = (int)strlen("migr_numa");
	if (w_lc < (int)strlen("migr_loc_core")) w_lc = (int)strlen("migr_loc_core");
	if (w_ll < (int)strlen("migr_loc_llc")) w_ll = (int)strlen("migr_loc_llc");
	if (w_lx < (int)strlen("migr_loc_xllc")) w_lx = (int)strlen("migr_loc_xllc");

	/* Top grouped header */
	int totals_block = w_tot + 1 + w_rw + 1 + w_lb + 1 + w_n;
	int reason_block = w_rw + 1 + w_lb + 1 + w_n;
	int locality_block = w_lc + 1 + w_ll + 1 + w_lx;
	printf("%-*s | %-*s | %-*s | %-*s\n",
		w_id, "id", totals_block, "totals", reason_block, "by_reason", locality_block, "by_locality");
	/* Bottom labels */
	printf("%-*s | %-*s %-*s %-*s %-*s | %-*s %-*s %-*s | %-*s %-*s %-*s\n",
		w_id, "paramset_id",
		w_tot, "migr_total", w_rw, "migr_wakeup", w_lb, "migr_lb", w_n, "migr_numa",
		w_rw, "migr_wakeup", w_lb, "migr_lb", w_n, "migr_numa",
		w_lc, "migr_loc_core", w_ll, "migr_loc_llc", w_lx, "migr_loc_xllc");
	/* Rows */
	key = next = 0;
	while ((err = bpf_map_get_next_key(stats_fd, &key, &next)) == 0) {
		struct schedscore_paramset_stats st;
...
static void dump_pid_migrations_matrix_table(struct schedscore_bpf *skel)
{
	...
	printf("%-8s  %-16s  | %-17s | %-17s | %-17s\n",
		"pid", "comm", "wakeup", "loadbalance", "numa");
	printf("%-8s  %-16s  | %-5s %-5s %-5s | %-5s %-5s %-5s | %-5s %-5s %-5s\n",
		"", "", "core","llc","xllc",  "core","llc","xllc",  "core","llc","xllc");
	...
	while ((err = bpf_map_get_next_key(pid_fd, &kpid, &npid)) == 0) {
		...
		if (bpf_map_lookup_elem(pid_fd, &npid, &v) == 0) {
			printf("%-8u  %-16.16s  | %5llu %5llu %5llu | %5llu %5llu %5llu | %5llu %5llu %5llu\n",
				npid, v.comm,
				v.migr_grid[SC_MR_WAKEUP][SC_ML_CORE], v.migr_grid[SC_MR_WAKEUP][SC_ML_LLC], v.migr_grid[SC_MR_WAKEUP][SC_ML_XLLC],
				v.migr_grid[SC_MR_LB][SC_ML_CORE],     v.migr_grid[SC_MR_LB][SC_ML_LLC],     v.migr_grid[SC_MR_LB][SC_ML_XLLC],
				v.migr_grid[SC_MR_NUMA][SC_ML_CORE],   v.migr_grid[SC_MR_NUMA][SC_ML_LLC],   v.migr_grid[SC_MR_NUMA][SC_ML_XLLC]);
		}
		kpid = npid;
	}
}
...
static int dump_json(struct schedscore_bpf *skel, const struct opts *o)
{
	...
	while ((err = bpf_map_get_next_key(stats_fd, &key, &next)) == 0) {
		...
		if (bpf_map_lookup_elem(stats_fd, &next, &st) == 0) {
			...
			compute_oncpu_quantiles(st.on_hist, &p50_on, &p95_on, &p99_on);
			unsigned int cnt = count_pids_for_paramset(pid2set_fd, next);
			unsigned long long r_w=0,r_lb=0,r_n=0,l_c=0,l_l=0,l_x=0;
			r_w  = st.migr_grid[SC_MR_WAKEUP][SC_ML_CORE] + st.migr_grid[SC_MR_WAKEUP][SC_ML_LLC] + st.migr_grid[SC_MR_WAKEUP][SC_ML_XLLC];
			r_lb = st.migr_grid[SC_MR_LB][SC_ML_CORE]     + st.migr_grid[SC_MR_LB][SC_ML_LLC]     + st.migr_grid[SC_MR_LB][SC_ML_XLLC];
			r_n  = st.migr_grid[SC_MR_NUMA][SC_ML_CORE]   + st.migr_grid[SC_MR_NUMA][SC_ML_LLC]   + st.migr_grid[SC_MR_NUMA][SC_ML_XLLC];
			l_c  = st.migr_grid[SC_MR_WAKEUP][SC_ML_CORE] + st.migr_grid[SC_MR_LB][SC_ML_CORE] + st.migr_grid[SC_MR_NUMA][SC_ML_CORE];
			...
			printf("%s    {\"paramset_id\":%u,\"details\":{\"policy\":\"%s\",\"nice\":%d,\"rtprio\":%u,\"dl_runtime_ns\":%llu,\"dl_deadline_ns\":%llu,\"dl_period_ns\":%llu,\"uclamp_min\":%u,\"uclamp_max\":%u,\"cgroup_id\":%llu,\"cpus_ranges\":\"%s\",\"cpus_weight\":%u,\"mems_ranges\":\"%s\",\"mems_weight\":%u},\"pids\":%u,\"p50_sched_latency_ns\":%.0f,\"avg_sched_latency_ns\":%.0f,\"p95_sched_latency_ns\":%.0f,\"p99_sched_latency_ns\":%.0f,\"p50_oncpu_ns\":%.0f,\"avg_oncpu_ns\":%.0f,\"p95_oncpu_ns\":%.0f,\"p99_oncpu_ns\":%.0f,\"nr_sched_periods\":%u,\"migrations\":{\"total\":%llu},\"migrations_by_reason\":{\"wakeup\":%llu,\"lb\":%llu,\"numa\":%llu},\"migrations_by_locality\":{\"core\":%llu,\"llc\":%llu,\"xllc\":%llu},\"migrations_grid\":{\"wakeup\":{\"core\":%llu,\"llc\":%llu,\"xllc\":%llu},\"lb\":{\"core\":%llu,\"llc\":%llu,\"xllc\":%llu},\"numa\":{\"core\":%llu,\"llc\":%llu,\"xllc\":%llu}}}\n",
				first?"":",", next,
				...
				(r_w+r_lb+r_n), r_w, r_lb, r_n, l_c, l_l, l_x,
				st.migr_grid[SC_MR_WAKEUP][SC_ML_CORE], st.migr_grid[SC_MR_WAKEUP][SC_ML_LLC], st.migr_grid[SC_MR_WAKEUP][SC_ML_XLLC],
				st.migr_grid[SC_MR_LB][SC_ML_CORE],     st.migr_grid[SC_MR_LB][SC_ML_LLC],     st.migr_grid[SC_MR_LB][SC_ML_XLLC],
				st.migr_grid[SC_MR_NUMA][SC_ML_CORE],   st.migr_grid[SC_MR_NUMA][SC_ML_LLC],   st.migr_grid[SC_MR_NUMA][SC_ML_XLLC]);
			first=0;
		}
		key = next;
	}
	...
	while ((err = bpf_map_get_next_key(pid_fd, &key, &next)) == 0) {
		if (bpf_map_lookup_elem(pid_fd, &next, &val) == 0) {
			...
			unsigned long long r_w=0,r_lb=0,r_n=0,l_c=0,l_l=0,l_x=0;
			r_w  = val.migr_grid[SC_MR_WAKEUP][SC_ML_CORE] + val.migr_grid[SC_MR_WAKEUP][SC_ML_LLC] + val.migr_grid[SC_MR_WAKEUP][SC_ML_XLLC];
			r_lb = val.migr_grid[SC_MR_LB][SC_ML_CORE]     + val.migr_grid[SC_MR_LB][SC_ML_LLC]     + val.migr_grid[SC_MR_LB][SC_ML_XLLC];
			r_n  = val.migr_grid[SC_MR_NUMA][SC_ML_CORE]   + val.migr_grid[SC_MR_NUMA][SC_ML_LLC]   + val.migr_grid[SC_MR_NUMA][SC_ML_XLLC];
			l_c  = val.migr_grid[SC_MR_WAKEUP][SC_ML_CORE] + val.migr_grid[SC_MR_LB][SC_ML_CORE] + val.migr_grid[SC_MR_NUMA][SC_ML_CORE];
			l_l  = val.migr_grid[SC_MR_WAKEUP][SC_ML_LLC]  + val.migr_grid[SC_MR_LB][SC_ML_LLC]  + val.migr_grid[SC_MR_NUMA][SC_ML_LLC];
			l_x  = val.migr_grid[SC_MR_WAKEUP][SC_ML_XLLC] + val.migr_grid[SC_MR_LB][SC_ML_XLLC] + val.migr_grid[SC_MR_NUMA][SC_ML_XLLC];
			printf("%s    {\"pid\":%u,\"comm\":\"%s\",\"paramset_id\":%u,\"p50_sched_latency_ns\":%.0f,\"avg_sched_latency_ns\":%.0f,\"p95_sched_latency_ns\":%.0f,\"p99_sched_latency_ns\":%.0f,\"p50_oncpu_ns\":%.0f,\"avg_oncpu_ns\":%.0f,\"p95_oncpu_ns\":%.0f,\"p99_oncpu_ns\":%.0f,\"nr_sched_periods\":%u,\"migrations\":{\"total\":%llu},\"migrations_by_reason\":{\"wakeup\":%llu,\"lb\":%llu,\"numa\":%llu},\"migrations_by_locality\":{\"core\":%llu,\"llc\":%llu,\"xllc\":%llu},\"migrations_grid\":{\"wakeup\":{\"core\":%llu,\"llc\":%llu,\"xllc\":%llu},\"lb\":{\"core\":%llu,\"llc\":%llu,\"xllc\":%llu},\"numa\":{\"core\":%llu,\"llc\":%llu,\"xllc\":%llu}}}\n",
				...
				(r_w+r_lb+r_n), r_w, r_lb, r_n, l_c, l_l, l_x,
				val.migr_grid[SC_MR_WAKEUP][SC_ML_CORE], val.migr_grid[SC_MR_WAKEUP][SC_ML_LLC], val.migr_grid[SC_MR_WAKEUP][SC_ML_XLLC],
				val.migr_grid[SC_MR_LB][SC_ML_CORE],     val.migr_grid[SC_MR_LB][SC_ML_LLC],     val.migr_grid[SC_MR_LB][SC_ML_XLLC],
				val.migr_grid[SC_MR_NUMA][SC_ML_CORE],   val.migr_grid[SC_MR_NUMA][SC_ML_LLC],   val.migr_grid[SC_MR_NUMA][SC_ML_XLLC]);
			first=0;
		}
		key = next;
	}
...
static void dump_migrations_csv(struct schedscore_bpf *skel, const struct opts *o)
{
	...
	printf("paramset_id,wk/core,wk/llc,wk/xllc,lb/core,lb/llc,lb/xllc,numa/core,numa/llc,numa/xllc\n");
	key = next = 0;
	while ((err = bpf_map_get_next_key(stats_fd, &key, &next)) == 0) {
		struct schedscore_paramset_stats st;
		if (bpf_map_lookup_elem(stats_fd, &next, &st) == 0) {
			printf("%u,%llu,%llu,%llu,%llu,%llu,%llu,%llu,%llu,%llu\n",
				next,
				st.migr_grid[SC_MR_WAKEUP][SC_ML_CORE], st.migr_grid[SC_MR_WAKEUP][SC_ML_LLC], st.migr_grid[SC_MR_WAKEUP][SC_ML_XLLC],
				st.migr_grid[SC_MR_LB][SC_ML_CORE],     st.migr_grid[SC_MR_LB][SC_ML_LLC],     st.migr_grid[SC_MR_LB][SC_ML_XLLC],
				st.migr_grid[SC_MR_NUMA][SC_ML_CORE],   st.migr_grid[SC_MR_NUMA][SC_ML_LLC],   st.migr_grid[SC_MR_NUMA][SC_ML_XLLC]);
	...
	printf("pid,wk/core,wk/llc,wk/xllc,lb/core,lb/llc,lb/xllc,numa/core,numa/llc,numa/xllc\n");
	key = next = 0;
	while ((err = bpf_map_get_next_key(pid_fd, &key, &next)) == 0) {
		struct schedscore_pid_stats val;
		if (bpf_map_lookup_elem(pid_fd, &next, &val) == 0) {
		...
		if (o->show_migration_matrix) {
			...
			while ((err = bpf_map_get_next_key(stats_fd, &k, &n)) == 0) {
				...
				if (bpf_map_lookup_elem(stats_fd, &n, &st) == 0) {
					printf("%-12u | %5llu %5llu %5llu | %5llu %5llu %5llu | %5llu %5llu %5llu\n",
						n,
						st.migr_grid[SC_MR_WAKEUP][SC_ML_CORE], st.migr_grid[SC_MR_WAKEUP][SC_ML_LLC], st.migr_grid[SC_MR_WAKEUP][SC_ML_XLLC],
						st.migr_grid[SC_MR_LB][SC_ML_CORE],     st.migr_grid[SC_MR_LB][SC_ML_LLC],     st.migr_grid[SC_MR_LB][SC_ML_XLLC],
						st.migr_grid[SC_MR_NUMA][SC_ML_CORE],   st.migr_grid[SC_MR_NUMA][SC_ML_LLC],   st.migr_grid[SC_MR_NUMA][SC_ML_XLLC]);
...
static int push_cpu_topology(struct schedscore_bpf *skel)
{
	...
	if (core_fd < 0 || llc_fd < 0)
		return -1;
	for (int cpu = 0; cpu < nproc; cpu++) {
		char p[256]; unsigned int core_id=0, pkg_id=0, llc_id=0, core_key=0, llc_key=0;
		...
		if (detect_llc_id(cpu, &llc_id) != 0)
			llc_id = (pkg_id << 16); /* fallback: package as LLC cluster */
		llc_key = llc_id;
		__u32 k = cpu;
		if (bpf_map_update_elem(core_fd, &k, &core_key, BPF_ANY))
			perror("cpu_core_id update");
		if (bpf_map_update_elem(llc_fd, &k, &llc_key, BPF_ANY))
			perror("cpu_llc_id update");
	}
	return 0;
}
