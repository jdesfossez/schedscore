...
int parse_opts(int argc, char **argv, struct opts *o, char ***target_argv)
{
	...
	if (!o || !target_argv) {
		fprintf(stderr, "Invalid arguments to parse_opts\n");
		return -1;
	}
	...
	*target_argv = NULL;
	...
	while ((c = getopt_long(argc, argv, "hd:p:n:g:G:l:wPFA:R:fu:e:o:D:M:C:",
				long_opts, NULL)) != -1) {
		switch (c) {
		case 'd':
			o->duration_sec = atoi(optarg);
			if (o->duration_sec < 0) {
		...
		case 'n':
			...
			if (!o->comm) {
				fprintf(stderr, "Failed to allocate memory for comm\n");
				return -1;
			}
			break;
		case 9: {
			...
			if (!ns) {
				fprintf(stderr, "Invalid wakeup latency: %s\n", optarg);
				return -1;
			}
		...
		case 'g':
			...
			if (!o->cgroup_path) {
				fprintf(stderr, "Failed to allocate memory for cgroup path\n");
				return -1;
			}
			break;
		case 'G':
			...
			break;
		case 'l':
			o->latency_warn_us = atol(optarg);
			if (o->latency_warn_us < 0) {
				fprintf(stderr, "Invalid latency warning threshold: %s\n", optarg);
				return -1;
			}
			break;
		case 'w':
		...
		case 'D':
			...
			if (!o->out_dir) {
				fprintf(stderr, "Failed to allocate memory for output dir\n");
				return -1;
			}
			break;
		case 'M':
		...
		case 'A':
			...
			if (!o->perf_args) {
				fprintf(stderr, "Failed to allocate memory for perf args\n");
				return -1;
			}
			break;
		case 'R':
			...
			if (!o->ftrace_args) {
				fprintf(stderr, "Failed to allocate memory for ftrace args\n");
				return -1;
			}
			break;
		case 'f':
			o->follow_children = true;
			break;
		case 1:
			o->aggregate_enable = false;
			break;
		case 2:
			o->paramset_recheck = true;
			break;
		case 3:
			o->timeline_enable = true;
			break;
		case 4:
			o->resolve_masks = false;
			break;
		case 5:
			o->show_hist_config = true;
			break;
		case 6:
			o->show_migration_matrix = true;
			break;
		case 7:
			o->show_pid_migration_matrix = true;
			break;
		case 8:
			o->dump_topology = true;
			break;
		case 'h':
